...
export class GenshinImpactService {
  ...
  private readonly act_id: string | null
  ...
  async checkIn (): Promise<any> {
    ...
    return await this.request('/sign', {
      ...
      onResponse ({ request, options, response }): Promise<void> | void {
        logger.debug({ request, options, response: response._data }, 'genshin impact check-in')

        if (response._data.retcode === -100) {
          throw new HoyolabCookieError('Cookie is invalid or expired', response._data.retcode)
        }

        if (response._data.retcode === -199 || response._data.retcode === -1005) {
  ...
  async getReCheckInMission (): Promise<any> {
    ...
    return await this.request('/task/list', {
      ...
      onResponse ({ request, options, response }): Promise<void> | void {
        logger.debug({ request, options, response: response._data }, 'genshin impact get missions')
      }
    })
  }
  ...
  async completeTask (taskId: number): Promise<any> {
    ...
    return await this.request('/task/complete', {
      ...
      onResponse ({ request, options, response }): Promise<void> | void {
        logger.debug({ request, options, response: response._data }, 'genshin impact complete task')
      }
    })
  }
  ...
  async claimAward (taskId: number): Promise<any> {
    ...
    return await this.request('/task/award', {
      ...
      onResponse ({ request, options, response }): Promise<void> | void {
        logger.debug({ request, options, response: response._data }, 'genshin impact claim award')
      }
    })
  }
  ...
  async reCheckIn (): Promise<any> {
    ...
    return await this.request('/resign', {
      ...
      body: { act_id: this.act_id },
      onResponse ({ request, options, response }): Promise<void> | void {
        logger.debug({ request, options, response: response._data }, 'genshin impact re-check-in')
      }
    })
  }
